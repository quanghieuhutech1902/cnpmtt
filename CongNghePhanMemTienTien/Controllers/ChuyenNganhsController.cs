...
namespace CongNghePhanMemTienTien.Controllers
{
    public class ChuyenNganhsController : Controller
    {
        ...
        private string pathFile = "/Files/CHUYEN-NGANH/" + DateTime.Now.Year + "/" + DateTime.Now.Month + "/Images/";
        ...
        public ActionResult Index(int? page, int? id)
        {
            ...
            var listAll = (from chuyenNganh in db.ChuyenNganhs
                           orderby chuyenNganh.ID descending
                           select chuyenNganh).ToPagedList(pageNum, pageSize);
            return View(listAll);
        }
        ...
        public ActionResult Create(ChuyenNganh chuyenNganh, HttpPostedFileBase Logo)
        {
            try
            {
                if (Logo != null)
                {
                    chuyenNganh.Logo = UploadCreateDir(Logo, "");
                }
            ...
            {
                ViewBag.NganhID = new SelectList(db.Nganhs, "ID", "TenNganh", chuyenNganh.NganhID);
                return View(chuyenNganh);
            } 
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(ChuyenNganh chuyenNganh, HttpPostedFileBase Logo)
        {
            try
            {
                if (Logo != null)
        ...
        public ActionResult Delete(int id)
        {
            try
            {
                var item = db.ChuyenNganhs.Find(id);
                db.ChuyenNganhs.Remove(item);
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            catch
